...
export function AddMenuDialog({ open, onOpenChange, parentId = null }: AddMenuDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await createMenu.mutateAsync({ name: name.trim(), icon, parentId });
    setName('');
    setIcon('file-text');
    onOpenChange(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Create New {parentId ? 'Sub-Menu' : 'Menu'}</DialogTitle>
          <DialogDescription>
            Add a new {parentId ? 'sub-menu' : 'menu'} to organize your data.
          </DialogDescription>
        </DialogHeader>
